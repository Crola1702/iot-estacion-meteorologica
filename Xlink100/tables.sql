...
CREATE TABLE IF NOT EXISTS measurements (
    ...
    PRIMARY KEY (time, station_id, slot)
);

SELECT create_hypertable('measurements', 'time', if_not_exists => TRUE);

CREATE INDEX IF NOT EXISTS idx_meas_station_slot_time
    ON measurements (station_id, slot, time DESC);

ALTER TABLE measurements SET (
    timescaledb.compress,
    timescaledb.compress_segmentby = 'station_id, slot'
);
...
FROM measurements
GROUP BY bucket, station_id, slot
WITH NO DATA;
...
INSERT INTO stations (id, name, organization, latitude, longitude, altitude_m, device_model)
VALUES (
    'uniandes-meteo-01',
...
INSERT INTO sensors (station_id, slot, label, quantity, unit) VALUES
    ('uniandes-meteo-01', 'M1', 'GHI',       'irradiance',   'W/m2'),
    ('uniandes-meteo-01', 'M2', 'Rad.',      'irradiance',   'W/m2'),
    ...
    ('uniandes-meteo-01', 'M7', 'Direcc.',   'wind_dir',     'deg'),
    ('uniandes-meteo-01', 'M8', 'Precip.',   'precipitation','mm/h')
